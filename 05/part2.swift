...
print(units.map { unit -> (String, Int) in
    (
        unit,
        string.replacingOccurrences(of: "\(unit)", with: "")
            .replacingOccurrences(of: "\(unit.uppercased())", with: "").react.count
    )
}.sorted { $0.1 < $1.1 }.first!.1)
